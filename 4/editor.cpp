...
void TextEditor::executeNormalCommand(InputKey k) {
  ...
  for (int i = 0; i < count; ++i) {
    ...
    // CHANGED: Handle / (search forward)
    case SingleCharCmd::SearchForward: {
      status.currMode = EditorMode::COMMAND;
      status.lastCommand = "/";
      return;
    }

    // CHANGED: Handle ? (search backward)
    case SingleCharCmd::SearchBackward: {
      status.currMode = EditorMode::COMMAND;
      status.lastCommand = "?";
      return;
    }
...
void TextEditor::executeCommandMode(const string &rawCmd) {
  ...
  // CHANGED: Handle backward search with ?
  if (!cmd.empty() && cmd[0] == '?') {
    string pattern = cmd.substr(1); // Extract pattern after '?'
    performSearch(pattern, false);  // Backward search
    status.currMode = EditorMode::NORMAL;
    return;
  }
