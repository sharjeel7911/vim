...
GapBuffer::GapBuffer(size_t initialCap) : bufferSize(initialCap ? initialCap : 8), gapStart(0), gapEnd(initialCap ? initialCap : 8) {
...
void GapBuffer::deleteTextInRange(size_t start, size_t end) {
  ...
  if ((gapEnd + count) > bufferSize) return;

  gapEnd += count;
}

void GapBuffer::replaceTextInRange(size_t start, size_t end, const char* replacement) {
...
void GapBuffer::moveCursorDown(size_t currentCol) {
  size_t currentRow = getRowIndexFromPos(gapStart);
  if (currentRow >= getRowCount() - 1) return;
...
void GapBuffer::moveCursorByWord(bool forward) {
  if (forward) {
    ...
    // backward logic (look at gapStart - 1), skip whitespace
...
void GapBuffer::moveCursorToEndOfWord() {
  ...
  while (gapStart < getTextLength() - 1 && isWordChar(getCharAt(gapStart + 1))) {
    moveCursorRight();
  }
}
...
size_t GapBuffer::getCurrentRowIndex() { return getRowIndexFromPos(gapStart) + 1; }
size_t GapBuffer::getCurrentColIndex() { return getColIndexFromPos(gapStart) + 1; }
...
size_t GapBuffer::getRowStartIndex(size_t rowIndex) {
  ...
  for (size_t i = 0; i < getTextLength(); i++) {
    if (getCharAt(i) == '\n') {
      count++;
      if (count == rowIndex)
        return i + 1; // next char after '\n' is start of line so returns index of first char of current line
    }
  }
...
size_t GapBuffer::findPreviousPatternFirstIndex(const char* pattern, size_t start) {
  size_t patternLen = strlen(pattern);
