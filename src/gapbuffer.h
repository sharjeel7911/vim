...
public:
  ...
  // constructor: allocate buffer at start, set gapStart = 0, gapEnd = capacity (whole buffer is gap)
  ...
  // for cursor to jump specific line like jump to line 110 (:110) : absolute jump
  ...
  // finds column number from a position in a row [we have to find the line start first to calculate column as buffer don't know anything about rows/columns, it just has a gap and chars]
  ...
  // finds next occurrence of searched pattern starting from index 'start' and returns its index or -1 if not found
  size_t findNextPatternFirstIndex(const char*, size_t);
  // finds previous occurrence of searched pattern starting from index 'start' and returns its index or -1 if not found
