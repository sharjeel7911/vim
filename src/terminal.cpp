...
InputKey Terminal::readKey() {
  ...
  if (c == '\x1b') {
    ...
    if (seq[0] == '[') {
      if (seq[1] >= '0' && seq[1] <= '9') {
        ...
        switch (seq[1]) {
        case 'A': return InputKey::ARROW_UP;
        case 'B': return InputKey::ARROW_DOWN;
        case 'C': return InputKey::ARROW_RIGHT;
        case 'D': return InputKey::ARROW_LEFT;
        }
      }
    }
  ...
  switch (c) {
  case 127: return InputKey::BACKSPACE;
...
void Terminal::render(TextEditor& editor) {
  ...
  rightSide << TEXT_MUTED << "┃ " << TEXT_WHITE << "Ln " << editor.status.cursorRow << TEXT_MUTED << ", " << TEXT_WHITE << "Col " << editor.status.cursorCol << "  " << modeColor << " " << editor.status.totalRows << " Lns " << RESET;

  int visibleLeftLen = modeStr.length() + 2 + fileInfo.length() + modifiedIndicator.length();
  int visibleRightLen = 2 + 3 + to_string(editor.status.cursorRow).length() + 2 + 4 + to_string(editor.status.cursorCol).length() + 2 + to_string(editor.status.totalRows).length() + 5;
